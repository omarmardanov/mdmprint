/* Скрипты лендинга 3D-стикеров. Подключается в конце body, DOM уже разобран.
  ...
  /* ГЕРОЙ (десктоп): четыре стикера лежат кучкой и по скроллу раскрываются веером.
     ...
     интерполяцией.
       ...
       blur — размытие задних планов, глубина. 24.08 ослаблено с 1.1/1.3/1.5 до 0.2/0.25/0.3 (в два захода)
              по правке маркетолога «на первом экране очень заблюрены стикеры»: на спутниках
              шириной 34–38% сцены прежние значения читались как мыло, а не как глубина;
  ...
  (function(){
    ...
    const OBJS = {
      ...
      a:{t:[.06,1],    d:22,
        from:{x:-64,y:-36, r:-12,s:.82, blur:.2, o:[22,64,106]},
        to:  {x:-98,y:-62, r:-20,s:.76, blur:0,   o:[46,30,104]}},
      b:{t:[.12,1],    d:24,
        from:{x:60, y:-42, r:13, s:.8,  blur:.25,o:[-30,44,106]},
        to:  {x:82, y:-62, r:20, s:.74, blur:0,   o:[-54,26,104]}},
      c:{t:[.18,1],    d:24,
        from:{x:88, y:66,  r:8,  s:.78, blur:.3, o:[14,70,106]},
  ...
  (function(){
    ...
    const paint = () => {
      ...
      let cur = null;
      /* 24.08: допуск был +4 и подсветка отставала на пункт. У секций
         scroll-margin-top = высота сжатой шапки + 12px, поэтому после перехода по якорю
         верх секции встаёт на 70px, а probe (нижняя кромка шапки) = 58: условие не
         выполнялось, и активным оставался предыдущий пункт. Замер по всем секциям:
         клик «Виды» подсвечивал «Бизнесу», «Вопросы» — «Контакты». Допуск = те же 12px + 4. */
      targets.forEach(s => { if(s.getBoundingClientRect().top <= probe + 16) cur = s; });
      links.forEach(a => {
  ...
  (function(){
    ...
    function build(key,labels){
      ...
      labels.forEach((it,i)=>{
        ...
        const inner = own
          ...
          // 24.08: картинка вставляется ТОЛЬКО там, где она есть (kind и mat). Раньше тег
          // создавался для всех шагов, и размер с тиражом давали 12 запросов в никуда —
          // onerror их прибирал, но 404 висели в консоли и в сети на каждой загрузке.
          : `<b class="opt__txt">${typeof it[0]==='number'?it[0].toLocaleString('ru-RU'):it[0]}${desc}</b>`+
            (['kind','mat'].includes(key)
    ...
    function calc(){
      ...
      /* 24.08: дата окончания сравнивалась с ЗАХАРДКОЖЕННЫМ 30 июля 2026, а не с сегодня —
         то есть условие было истинно всегда и акция не выключилась бы ни 1 сентября,
         ни позже: скидка продолжила бы считаться, а плашка «до 31 августа» врать.
         Сравниваем с текущей датой, конец дня включительно. */
      ...
      /* Плашка акции в первом экране свёрстана статикой. Если срок вышел, её надо убрать —
         иначе страница обещает скидку, которой в расчёте уже нет. */
      const heroPromo=document.querySelector('.hero__promo');
      if(heroPromo) heroPromo.hidden = !promoLive;
      const mark=$('calcMark');
